<div>
    ...
    <div class="purple-keeep max-w-6xl mx-auto mt-6 sm:px-6 lg:px-8">
        @if ($reply->user->id === Auth::user()->id && is_null($reply->started_at))
            <p class="p-1 text-center text-lg font-semi-bold">Rappel de votre code à communiquer: {{$reply->starting_code}}</p>
        @elseif ($reply->user->id != Auth::user()->id && !is_null($reply->started_at) && is_null($reply->ended_at))
            <p class="p-1 text-center text-lg font-semi-bold">Rappel de votre code à communiquer: {{$reply->ending_code}}</p>
        @endif
        <p>{{$status}}</p>
    </div>
    <div class="p-1 max-w-6xl pb-10 mx-auto mt-6 sm:px-6 lg:px-8">
        <div class="flex flex-wrap justify-center pt-2 pb-10 overflow-hidden shadow-sm">
            {{-- verif si trop d'essaie pour le premier code --}}
                ...
                @if ($reply->user->id != Auth::user()->id && is_null($reply->started_at))
                    <div class="flex-column">
                        <p class="text-3xl font-semi-bold">Saisie du code: </p>
                            <!-- Saisie du code -->
                            <div class=" mt-3">
                        ...
                        <p class="purple-keeep p-2 text-xl">Vérifier que le code saisi correspond à celui qui vous a été donné.</p>
                        @if ($reply->starting_code_count)
                            <p class="p-2 text-xl text-red-600">Code invalide</p>
                        @endif
                        <p class="p-2 text-xl text-red-600">Il vous reste {{3 - $reply->starting_code_count}} essaies.</p>
                    </div>
                ...
                @elseif (!is_null($reply->ended_at))
                    <div class="purple-keeep flex-column">
                        <p class="p-1 text-4xl font-bold">La transaction est terminé</p>
                    </div>
                    {{-- S'affiche quand la transaction est en cours pour le partie qui n'a pas a saisir de code  --}}
                @else
                    <div class="purple-keeep flex-column">
                        <p class="p-1 text-4xl font-bold">La transaction est en cours</p>
                    </div>
                @endif
        </div>
    </div>
