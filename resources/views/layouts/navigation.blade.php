<nav x-data="{ open: false }" class="bg-white">
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                <!-- Logo -->
                {{-- <div class="flex items-center flex-shrink-0 uppercase">
                    {{ $header }}
                </div> --}}
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    <x-nav-link :href="route('reply.myreplies')" :active="request()->routeIs('reply.myreplies')">
                        {{ __('Mes transactions') }}
                    </x-nav-link>
                    ...
                    <a href="{{ route('reply.index') }}" class="{{ request()->routeIs('reply.index') ? 'inline-flex items-center px-1 pt-1 border-b-2 border-indigo-400 text-sm font-medium leading-5 text-gray-900 focus:outline-none focus:border-indigo-700 transition duration-150 ease-in-out' : 'inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium leading-5 text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition duration-150 ease-in-out'}}">
                        ...
                        @if (Auth::user()->getRepliesCount() > 0)
                            <span class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full">
                                {{ Auth::user()->getRepliesCount() }}
                            </span>
                        @endif
                    </a>
                    @endcan
            ...
            <div class="burger-menu flex items-center -mr-2 w-full relative sm:hidden">
                <button @click="open = ! open" class="absolute inline-flex items-center justify-center text-gray-400 transition duration-150 ease-in-out rounded-md hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 focus:text-gray-500">
                    <svg class="w-6 h-6" stroke="#341074" fill="none" viewBox="0 0 24 24">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1 nav-mobile">
            ...
            <x-responsive-nav-link :href="route('reply.myreplies')" :active="request()->routeIs('reply.myreplies')">
                {{ __('Mes transactions') }}
            </x-responsive-nav-link>
