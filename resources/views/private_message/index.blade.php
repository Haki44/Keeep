                ...
                <div id="messages" class="flex flex-col-reverse p-3 space-y-4 overflow-y-auto scrolling-touch scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2">
                    ...
                    @foreach ($private_messages as $private_message)
                        @if ($private_message->to_id == $user->id)
                            {{-- <div class="chat-message">
                                <div class="flex items-end justify-end">
                                    <div class="flex flex-col items-end order-1 max-w-xs mx-2 space-y-2 text-xs">
                                        <div class="flex flex-col"><span class="inline-block px-4 py-2 text-white bg-blue-600 rounded-lg rounded-br-none ">{{ $private_message->content }}</span><span>{{ date('H:i', strtotime($private_message->created_at))}}</span></div>
                                    </div>
                            ...
                            <x-chat-message :pm="$private_message" type="sender" />
                        @else
                            <x-chat-message :pm="$private_message" type="reciever" />
                            {{-- <div class="chat-message">
                                <div class="flex items-end">
                                    <div class="flex flex-col items-start order-2 max-w-xs mx-2 space-y-2 text-xs">
                                        <div class="flex flex-col"><span class="inline-block px-4 py-2 text-gray-600 bg-gray-300 rounded-lg rounded-bl-none">{{ $private_message->content }}</span><span>{{ date('H:i', strtotime($private_message->created_at))}}</span></div>
                                    </div>
